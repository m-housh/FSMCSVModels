...
class FSMPriceBookTests: XCTestCase {
    ...
    func testQuantityUnitInit() {
    
        for unit in FSMQuantityUnit.all {
            let strongUnit = FSMQuantityUnit(unit.description)
            XCTAssertNotNil(strongUnit)
        }
    }
    
    func testQuantityUnitReturnNil() {
        var string = "Too Many Words"
        XCTAssertNil(FSMQuantityUnit(string))
